...
export const CoverageSection: React.FC = () => {
  ...
  return (
    <section id="coverage" className="py-20 bg-slate-900 text-slate-50 border-t border-slate-800 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-3">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-sm border transition duration-200 cursor-pointer flex items-center justify-between ${
                    isSelected
                      ? 'bg-slate-950 border-2 border-orange-500 text-white shadow-xl'
                ...
                >
                  <div className="space-y-1">
                    <div className="font-black text-sm sm:text-base flex items-center gap-2 uppercase tracking-tight">
                      <MapPin className={`w-4 h-4 ${isSelected ? 'text-orange-400' : 'text-slate-500'}`} />
                      <span>{hub.county}</span>
                    </div>
                    <div className="text-xs text-slate-400">
                      Anchor Hub: <span className="text-slate-200 font-semibold">{hub.hubCity}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-[11px] font-mono px-2 py-0.5 rounded-sm bg-slate-900 text-orange-300 border border-slate-800 font-bold">
            ...
            <div className="p-4 rounded-sm bg-slate-950 border border-slate-800 text-xs text-slate-400 space-y-1.5 mt-4">
              <div className="font-black uppercase tracking-wider text-slate-200 flex items-center gap-1.5 text-[11px]">
                <Navigation className="w-3.5 h-3.5 text-orange-400" />
                Physical Headquarters & Anchor:
              </div>
              <div className="font-bold text-white">{BUSINESS_ENTITY.brandName}</div>
              <div className="text-slate-300 font-mono text-[11px]">{BUSINESS_ENTITY.address.fullFormatted}</div>
          ...
          <div className="lg:col-span-7 bg-slate-950 rounded-sm border border-slate-700 p-7 shadow-2xl">
            <div className="border-b border-slate-800 pb-5 mb-6">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                <span className="text-xs font-bold uppercase tracking-widest text-orange-400">
                  Target County Profile
                </span>
                <span className="text-xs font-mono px-3 py-1 rounded-sm bg-orange-600/20 text-orange-300 border border-orange-500/30 font-bold">
                  Wallingford Distance: {activeHub.distanceFromWallingford}
                </span>
              </div>
              <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tight uppercase">{activeHub.county}</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                {activeHub.towns.map((town) => (
                  <div
                    key={town}
                    className="p-2.5 rounded-sm bg-slate-900 border border-slate-800 text-center text-xs font-bold text-slate-200 hover:border-orange-500/50 transition"
                  >
                    {town}
                  </div>
                ))}
            ...
            <div className="mt-8 pt-6 border-t border-slate-800 space-y-2 text-xs text-slate-400">
              <div className="font-black uppercase tracking-wider text-slate-300 text-[11px]">

...
export const Navbar: React.FC<NavbarProps> = ({ onOpenAudit }) => {
  ...
  const navLinks = [
    ...
    { name: 'SEO Company', href: '/seo-company-connecticut/' },
    { name: 'SEO Agency', href: '/seo-agency-connecticut/' },
    ...
    <header className="sticky top-0 z-40 bg-slate-950/95 backdrop-blur-md border-b border-slate-800/80 text-slate-50 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden lg:flex items-center gap-6 xl:gap-9 text-xs xl:text-sm font-bold uppercase tracking-wider text-slate-300">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  to={link.href}
                  className={`hover:text-orange-400 transition-colors py-2 ${location.pathname === link.href ? 'text-orange-400' : ''}`}
                >
                  {link.name}
                </Link>
              );
            ...
            >
              <button className={`flex items-center gap-1 hover:text-orange-400 transition-colors ${location.pathname.includes('-ct') ? 'text-orange-400' : ''}`}>
                Locations <ChevronDown className="w-4 h-4" />
              </button>
              
              {locationsOpen && (
                <div className="absolute top-full left-0 w-48 bg-slate-900 border border-slate-800 rounded-sm shadow-xl py-2 z-50">
                  {locationLinks.map((loc) => (
                    <Link
                      key={loc.name}
                      to={loc.href}
                      className="block px-4 py-2 hover:bg-slate-800 hover:text-orange-400 transition-colors"
                    >
                      {loc.name}
                    </Link>
                  ))}
            ...
            <Link
              to="/contact/"
              className={`hover:text-orange-400 transition-colors py-2 ${location.pathname === '/contact/' ? 'text-orange-400' : ''}`}
            >
              Contact
            </Link>
          </nav>
          ...
          <div className="hidden sm:flex items-center gap-4">
            <button
              onClick={onOpenAudit}
              id="nav-audit-button"
              className="bg-orange-600 hover:bg-orange-500 active:scale-95 text-white font-black text-xs sm:text-sm tracking-tight rounded-sm px-6 py-3 uppercase shadow-lg shadow-orange-950/60 transition cursor-pointer flex items-center gap-2 group shrink-0"
            >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-slate-950 border-b border-slate-800 px-4 pt-3 pb-6 space-y-4 animate-in fade-in slide-in-from-top-2 duration-150">
          <nav className="flex flex-col space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className="px-3 py-2.5 rounded-sm text-sm font-bold uppercase tracking-wide text-slate-200 hover:text-orange-400 hover:bg-slate-900 transition"
              >
                {link.name}
              </Link>
            ))}
            <div className="px-3 py-2.5">
              ...
              <div className="flex flex-col ml-4 border-l border-slate-800 space-y-1">
                {locationLinks.map((loc) => (
                  <Link
                    key={loc.name}
                    to={loc.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="px-4 py-2 rounded-sm text-sm font-bold uppercase tracking-wide text-slate-300 hover:text-orange-400 hover:bg-slate-900 transition"
                  >
                    {loc.name}
                  </Link>
                ))}
